...
RobitLabling::RobitLabling(const cv::Mat& Img,
...
vector<cv::Rect> RobitLabling::getRecBlobs()
{
  return m_recBlobs;
}

unsigned int RobitLabling::getNumOfBlobs()
{
  return m_nBlobs;
}

void RobitLabling::sortingRecBlobs()
{
  if(m_nBlobs < 2)
    return;
...
void RobitLabling::_labeling_four_neighbor()
{
  __dynamicAllocation();
...
void RobitLabling::_labeling_eight_neighbor()
{
  __dynamicAllocation();
...
void RobitLabling::_quick_sort_rec_blobs(int left, int right)
{
  int L = left;
  int R = right;
...
void RobitLabling::__dynamicAllocation()
{
...
void RobitLabling::__freeAllocation()
{
  delete[]          m_isVisited;
  delete[]          m_visited_pt;
...
const unsigned int RobitLabling::__check_four_neighbor(cv::Point &start_pt, cv::Point &end_pt)
{
...
const unsigned int RobitLabling::__check_eight_neighbor(cv::Point &start_pt, cv::Point &end_pt)
{
  cv::Point present_pt = start_pt;
  m_area=1;
